		...
		if(!in_array($username, $userarray))
		{
			...
			if($maxtime == null || $maxtime < $newdate)
			{
				...
				if($timestamp == NULL || $timestamp < $newdate)
				{
					...
					$message .= '&nbsp;&nbsp;Log in to your <a href="ezdia.com?utm_source=katrina_sk" target="_blank">eZdia account</a>.<br>';
					...
					$message .= 'With best regards,<br>eZdia Team<br>Trusted Experts for your Small Needs<br><br>';
					$message .= '</div>';
					...
					$message .= '<br>Thanks and Regards,<br>eZdia Team';
					$message .= '</div>';
					$message .= '</div>';
					$message .= '</div>';
					$message .= '</body></html>';
